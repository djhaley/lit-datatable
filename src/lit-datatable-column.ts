...
export class LitDatatableColumn extends LitElement {
	...
	updated(properties: PropertyValues) {
		...
		if (properties.has("type") || properties.has("sort")) {
			if (this.type === "sort") {
				this.html = (value: string, p: string) =>
					html` <ld-header-with-sort
						data-property="${p}"
						@direction-changed="${this.handleSortDirectionChanged.bind(
							this,
							p
						)}"
		...
		if (properties.has("type") || properties.has("filterValue")) {
			if (this.type === "filter") {
				this.html = (value: string, p: string) =>
					html` <ld-header-with-filter
						.active="${!!this.filterValue}"
						.filterValue="${this.filterValue}"
						.header="${value}"
						.property="${p}"
						@filter-value-changed="${this.handleFilterTextChanged.bind(
							this
						)}"
					>
						${value}
		...
		if (
			...
			properties.has("selectedChoices")
		) {
			if (this.type === "choices") {
				this.html = (value: string, p: string) =>
					html` <ld-header-with-choices
						.choices="${this.choices}"
						.enableFilter=${this.enableFilter}
						.selectedChoices="${this.selectedChoices}"
						.property="${p}"
						@selected-choices-changed="${this.handleFilterChoiceChanged.bind(
							this
						)}"
					>
						${value}
		...
		if (
			...
			if (this.type === "dateSort") {
				this.html = (value: string, p: string) =>
					html` <ld-header-with-date-and-sort
		...
		if (
			properties.has("type") ||
			properties.has("sort") ||
			properties.has("filterValue")
		) {
			if (this.type === "filterSort") {
				this.html = (value: string, p: string) =>
					html` <ld-header-with-filter-and-sort
						.active="${!!this.filterValue}"
						.filterValue="${this.filterValue}"
						data-property="${p}"
						.property="${p}"
	...
	handleSortDirectionChanged(
		...
		if (detail.value) {
			...
			this.dispatchEvent(
				new CustomEvent("sort", { detail: { value: this.sort } })
			);
